text_col <- function(x) {
    ...
    if (!isAvailable() || !hasFun("getThemeInfo")) {
        return(x)
    }
...
package_version <- function(x) {
    ...
    version <- unclass(packageVersion(x))[[1]]
    ...
    if (length(version) > 3) {
        version[4:length(version)] <- red(as.character(version[4:length(version)]))
    }
...
# Check which of the main chevreul packages
